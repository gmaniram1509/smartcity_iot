...
function Title({ children, colour = "#00d4ff" }) {
  return (
    <div style={{ fontFamily: "'Space Mono',monospace", fontSize: 9, letterSpacing: 3, textTransform: "uppercase", color: "#4a6a7a", marginBottom: 12, display: "flex", alignItems: "center", gap: 8 }}>
      <div style={{ width: 4, height: 4, borderRadius: "50%", background: colour, boxShadow: `0 0 8px ${colour}` }} />
      {children}
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div style={{ position:"relative", zIndex:1 }}>
        ...
        <header style={{ position:"sticky", top:0, zIndex:100, background:"rgba(5,10,15,0.94)", backdropFilter:"blur(20px)", borderBottom:"1px solid #1a3040", padding:"12px 28px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div style={{ display:"flex", alignItems:"center", gap:16 }}>
            <div style={{ fontFamily:"'Space Mono',monospace", fontSize:22, fontWeight:700, color:"#00d4ff", textShadow:"0 0 24px rgba(0,212,255,0.5)", letterSpacing:3 }}>
              NEXUS<span style={{ color:"#c8dde8" }}>CITY</span>
            </div>
            <div style={{ fontSize:10, letterSpacing:3, textTransform:"uppercase", color:"#4a6a7a", borderLeft:"1px solid #1a3040", paddingLeft:16 }}>
              Smart IoT Monitoring
            </div>
          </div>
          <div style={{ display:"flex", gap:24, alignItems:"center" }}>
            {[["Sensors", Object.keys(readings).length], ["Alerts", events.length], ["Polls", pollCount], ["Updated", lastPoll ? lastPoll.toLocaleTimeString() : "--"]].map(([label, value]) => (
              <div key={label} style={{ textAlign:"center" }}>
                <div style={{ fontFamily:"'Space Mono',monospace", fontSize:13, color:"#00d4ff" }}>{value}</div>
                <div style={{ fontSize:9, letterSpacing:2, textTransform:"uppercase", color:"#4a6a7a" }}>{label}</div>
              </div>
            ))}
        ...
        {critCount > 0 && (
          <div style={{ background:"rgba(255,59,92,0.12)", borderBottom:"1px solid rgba(255,59,92,0.4)", padding:"8px 28px", display:"flex", alignItems:"center", gap:10, fontFamily:"'Space Mono',monospace", fontSize:11, color:"#ff3b5c", letterSpacing:2 }}>
            🚨 {critCount} CRITICAL ALERT{critCount > 1 ? "S" : ""} — {events.find(e => e.severity === "CRITICAL")?.message?.slice(0, 80)}...
          </div>
        )}
          ...
          <div style={{ gridColumn:"1/4" }}>
            <Card>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:10 }}>
                {Object.entries(SENSOR_META).map(([sid, meta]) => {
            ...
            <Card style={{ height:280 }}>
              ...
              <svg width="100%" height="220" style={{ overflow:"visible" }}>
                {history.length > 1 ? (() => {
                  ...
                  ].map(({ key, colour, label }, li) => {
                    const vals = history.map(p => p[key] || 0).filter(v => v > 0);
                    if (!vals.length) return null;
                    const min = Math.min(...vals), max = Math.max(...vals) || 1;
                    const pts = history.map((p, i) => {
                      const x = pad + (i / (history.length - 1)) * (W - pad * 2);
                      const y = H - pad - ((p[key] || 0) - min) / (max - min) * (H - pad * 2);
                      return `${x},${y}`;
                    }).join(" ");
                    return (
                      <g key={key}>
                        <polyline points={pts} fill="none" stroke={colour} strokeWidth="2" style={{ filter:`drop-shadow(0 0 4px ${colour})` }} />
                        <text x={W - 80} y={14 + li * 16} fill={colour} fontSize="9" fontFamily="Space Mono">{label}</text>
                      </g>
                    );
                  });
          ...
          <div style={{ gridColumn:"3" }}>
            <Card>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                {[["Zone_A","ZONE A","Residential"],["Zone_B","ZONE B","Industrial"],["Zone_C","ZONE C","Commercial"],["Zone_D","ZONE D","Transport"]].map(([key, label, sub]) => {
                  const status = summary[key]?.status || "GOOD";
                  const colour = status === "CRITICAL" ? "#ff3b5c" : status === "WARNING" ? "#ffd600" : "#00ff9d";
                  return (
                    <div key={key} style={{ background:`${colour}08`, border:`1px solid ${colour}40`, borderRadius:4, padding:12, textAlign:"center", animation: status === "CRITICAL" ? "blink 1.2s infinite" : "none" }}>
                      <div style={{ width:8, height:8, borderRadius:"50%", background:colour, boxShadow:`0 0 10px ${colour}`, margin:"0 auto 6px" }} />
                      <div style={{ fontFamily:"'Space Mono',monospace", fontSize:11, letterSpacing:2, color:"#c8dde8", marginBottom:3 }}>{label}</div>
                    ...
                    {events.slice(0, 9).map((evt, i) => {
                      ...
                      return (
                        <div key={i} style={{ borderLeft:`3px solid ${c}`, background:`${c}08`, padding:"10px 12px", borderRadius:"0 4px 4px 0" }}>
                          ...
                          <div style={{ fontSize:10, color:"#8aacba", lineHeight:1.4 }}>{evt.message?.slice(0,80)}{(evt.message?.length||0)>80?"...":""}</div>
            ...
            <Card>
              ...
              <div style={{ display:"flex", alignItems:"center", overflowX:"auto" }}>
                {[
                  ...
                  { icon:"🗄️", name:"DynamoDB",    value:`${Object.values(readings).reduce((s,a)=>s+(a?.length||0),0)} records` },
                  { icon:"📊", name:"Dashboard",   value:`Poll #${pollCount}`, highlight:true },
                ].map((node, i, arr) => (
                  <div key={i} style={{ display:"flex", alignItems:"center", flexShrink:0 }}>
                    <div style={{ background: node.highlight ? "rgba(0,212,255,0.08)" : "#0f1e2e", border:`1px solid ${node.highlight ? "#00d4ff" : "#1a4060"}`, borderRadius:4, padding:"10px 14px", textAlign:"center", minWidth:100, boxShadow: node.highlight ? "0 0 16px rgba(0,212,255,0.15)" : "none" }}>
